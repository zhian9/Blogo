...
import {
  ...
  CodeOutlined, CheckCircleOutlined, SyncOutlined, PauseCircleOutlined, InboxOutlined,
  GithubOutlined,
} from '@ant-design/icons'
import { useCreateProject, useUpdateProject, useProjectBySlug } from '../hooks/useProjects'
...
export default function PublishProject() {
  ...
  useEffect(() => {
    if (project && isEdit) {
      ...
      if (project.visible_users) {
        const vuUsers: SelectedUser[] = (project.visible_users as any[]).map((vu: any) => ({
          id: vu.user_id,
          username: '',
          name: '',
        }))
    ...
    form.validateFields().then(async (values) => {
      ...
      try {
        if (isEdit && project) {
          ...
          setTimeout(() => navigate(`/project/${payload.slug || project.slug}`), 400)
        } else {
          const res = await createProject.mutateAsync(payload)
          const slug = res?.data?.slug
          if (!slug) { message.error('发布失败：未获取到项目链接'); setSubmitting(false); return }
          message.success(status === 'published' ? '发布成功！' : '已保存为草稿')
          setTimeout(() => navigate(`/project/${slug}`), 400)
        }
      } catch (err: any) {
  ...
  }, [form, content, visibility, isEdit, project, createProject, updateProject, navigate])

  const isPending = createProject.isPending || updateProject.isPending || submitting
  ...
  return (
    <div style={{ background: c.bg, minHeight: '100vh' }}>
      ...
      <Form form={form} layout="vertical" initialValues={{ status: 'draft', is_top: false, visibility: 'public', project_state: 'developing', content: '' }}>
        {/* ===== Title ===== */}
        ...
        {/* ===== Editor ===== */}
        <div>
          <Form.Item name="content" rules={[{ required: true, message: '请输入项目内容' }]} style={{ display: 'none' }}>
            <Input />
          </Form.Item>
        ...
        <div style={{ padding: '48px 5vw 48px' }}>
          ...
          >
            ...
            {/* ── 项目状态 ── */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ marginBottom: 8, color: c.textMuted, fontSize: 13 }}>项目状态</div>
              <Form.Item name="project_state" style={{ marginBottom: 0 }}>
                <Segmented
                  style={{
                    background: c.surfaceAlt,
                    border: `1px solid ${c.border}`,
            ...
            <Space size="large" style={{ width: '100%', marginBottom: 8 }}>
              <Form.Item name="github_url" label={<span style={{ color: c.textMuted }}><GithubOutlined /> GitHub 链接</span>} style={{ marginBottom: 12, minWidth: 280, flex: 1 }}>
                <Input placeholder="https://github.com/user/repo" maxLength={512} style={{ borderRadius: 8 }} />
              </Form.Item>
              <Form.Item name="demo_url" label={<span style={{ color: c.textMuted }}><LinkOutlined /> 在线演示</span>} style={{ marginBottom: 12, minWidth: 280, flex: 1 }}>
                <Input placeholder="https://demo.example.com" maxLength={512} style={{ borderRadius: 8 }} />
              </Form.Item>
            </Space>
            ...
            <Form.Item name="slug" label={<span style={{ color: c.textMuted }}><LinkOutlined /> Slug</span>}
              ...
              <Input placeholder="项目 URL 别名" addonBefore="/project/" maxLength={255} style={{ borderRadius: 8 }} />
            </Form.Item>

            <Form.Item name="summary" label={<span style={{ color: c.textMuted }}><FileTextOutlined /> 摘要</span>} style={{ marginBottom: 12 }}>
              <TextArea rows={2} maxLength={1000} showCount placeholder="项目摘要，留空自动生成" style={{ borderRadius: 8 }} />
            </Form.Item>
